...
struct HomeView: View {
    ...
    var spentToday: Double {
        let calendar = Calendar.current
        return transactions.filter { calendar.isDateInToday($0.date) && $0.amount > 0 }.reduce(0) { $0 + $1.amount }
    }
    ...
    var spentThisMonth: Double {
        let calendar = Calendar.current
        return transactions.filter { calendar.isDate($0.date, equalTo: Date(), toGranularity: .month) && $0.amount > 0 }.reduce(0) { $0 + $1.amount }
    }
                            ...
                            VStack(alignment: .leading, spacing: 24) {
                                HStack(alignment: .lastTextBaseline) {
                                    Text("ALL SUBSCRIPTIONS")
                                        ...
                                        .foregroundColor(Color.onSurfaceVariant)
                                    Spacer()
                                    Text(String(format: "%@%.2f/mo", currencySymbol, totalMonthlyFixedCost))
                                        .font(.custom("Inter", size: 14).weight(.bold))
